...
cycol = cycle('bgrcmky')
colors = ["#E9D8A6","#005F73","#94D2BD","#EE9B00","brown","teal"]
...
class DataGeneratorX:
  ...
  def plot_data(self,name):
      ...
      for i,t, (t_idx, t_val) in zip(range(len(self.targets)),self.targets, self.target_data):
        x_data = self.input_data[0][t_idx,:]
        y_data = np.random.uniform(-.1, .1, x_data.shape)
        plt.scatter(x_data, y_data, label=t, c=colors[i])
      plt.ylim(-1,1)
      plt.xlabel('x')
      plt.ylabel('Random vals')
      plt.title(name)
...
class DataGeneratorXT:
  ...
  def __init__(self, 
    ...
    self.target_data = None
    self.set_data()
    self.labels = ['Domain', 'BC $x=0$', 'BC $x=1$', 'IC $t=0$']
  ...
  def generate_uniform_T_samples(self, num_sample):
    if self.logT is True:
      t_dom = np.random.uniform(np.log(self.Tdomain[0]+1)/np.log(self.base), np.log(self.Tdomain[1]+1)/np.log(self.base), num_sample)
    else:
  ...
  def generate_data(self):
    ...
    num_sample = int(self.num_sample*0.75)
    ...
    num_sample = int(self.num_sample/8)
  ...
  def plot_data(self,name,scaled_ax=False):
      import seaborn as sns
      import datetime
      plt.close()
      fig = plt.figure(figsize=(6,6))
      ax = fig.add_subplot()
      cmap = sns.color_palette("bright",len(self.targets))
      sns.color_palette("bright",6)
      # cmap = sns.color_palette('hls',len(self.targets))
      i = 0
      label_dict= {'domain': "Domain", 'bc-left': r"BC $x_{min}$", 'bc-right': r"BC $x_{max}$",'bc-bot': r"BC $y_{min}$",'bc-top': r"BC $y_{max}$",'ic': "IC"}
      label_dict= {'domain': r"Domain",
                   'bc-left': r"BC ($\bar{x}=0$)",
                   'bc-right': r"BC ($\bar{x}=1$)",
                   'bc-bot': r"BC ($\bar{y}=0$)",
                   'bc-top': r"BC ($\bar{y}=0$)",
                   'ic': r"IC ($\bar{t}=0$)"}
      for i,t, (t_idx, t_val) in zip(range(len(self.targets)),self.targets, self.target_data):
        ...
        if t=="bc-left" or t=="bc-top" or t=="IC":
            zorder=3
            a = 1
        elif t=="domain":
            zorder=2
            ...
            zorder=3
        s=6
        ax.scatter(x_data, t_data, label=label_dict[t], c=cmap[i],s=s,zorder=zorder,alpha=a)
        i+=1
      ax.set_xlabel('x')
      ax.set_ylabel('y')
      if scaled_ax==True: ax.axis("scaled")
      # plt.title(name)
      # plt.legend(title="Collocation Points", bbox_to_anchor=(-0.1,1), loc='upper left')
      fig.tight_layout()
      plt.show()
  ...
  def plot_data(self,name):
      ...
      for t, (t_idx, t_val) in zip(self.targets, self.target_data):
        ...
        if t=="bc-left" or t=="bc-top" or t=="IC":
            ...
            zorder=3
        s=6
        ax.scatter(x_data, y_data, t_data, label=label_dict[t], c=cmap[i],s=s,zorder=zorder,alpha=a)
        i+=1
      ax.set_xlabel('x')
      ax.set_ylabel('y')
      ax.set_zlabel('t')
      # plt.title(name)
      plt.legend(title="Collocation Points", bbox_to_anchor=(0,1)
                  , loc='upper left'
                 )
      fig.tight_layout()
      plt.savefig("plots/Pasted_image_%s_GT_XYT.png"%datetime.datetime.now().strftime("%Y%m%d%H%M%S"),dpi=300)
      plt.show()
